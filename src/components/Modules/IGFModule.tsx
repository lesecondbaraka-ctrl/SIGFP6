...
import { Shield, AlertTriangle, CheckCircle, Eye, FileText, Clock, BarChart3 } from 'lucide-react';
...
export default function IGFModule() {
  ...
  // Filtre risque (utilise selectedRisque)
  const filteredTransactions = selectedRisque === 'Tous'
    ? transactions
    : transactions.filter(t => t.risque === selectedRisque);
          ...
          {activeTab === 'surveillance' && (
            <div className="space-y-6">
              ...
              <div className="flex flex-wrap items-center gap-2 text-xs">
                {getStatutBadge('En attente validation')}
                {getStatutBadge('Validé IGF')}
                {getStatutBadge('Rejeté IGF')}
                {getStatutBadge('Sous surveillance')}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white p-6 rounded-lg border border-gray-200">
                  ...
                  <div className="space-y-3">
                    {['MIN-INFRA', 'MIN-SANTE', 'MIN-EDUC', 'MIN-MINES'].map((entite) => {
                ...
                <div className="bg-white p-6 rounded-lg border border-gray-200">
                  ...
                  <div className="mb-3">
                    <label className="block text-xs text-gray-600 mb-1">Filtrer par risque</label>
                    <select
                  ...
                  <div className="space-y-3">
                    {['Élevé', 'Moyen', 'Faible'].map((risque) => {
                      const count = filteredTransactions.filter(t => t.risque === risque).length;
                      const percentage = (count / (filteredTransactions.length || 1)) * 100;
                      return (
                        <div key={risque}>
                          <div className="flex justify-between items-center mb-1">
                            <span className="text-sm text-gray-600">{getRisqueBadge(risque)}</span>
                            <span className="text-sm font-medium">{count}</span>
                          </div>
                          <div className="w-full bg-gray-200 rounded-full h-2">
                            <div 
                              ...
                              data-width={Math.round(percentage / 5) * 5}
                            ></div>
                          </div>
                        </div>
                      );
